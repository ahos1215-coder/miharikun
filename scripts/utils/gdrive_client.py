...
def _build_drive_service():
    ...
    try:
        ...
        print(f"[GDrive] 認証エラー: {e}。ローカル保存にフォールバック。")
        return None
...
def _save_local(content: str, filename: str) -> str:
    ...
    print(f"[GDrive] ローカル保存: {local_path}")
...
def upload_text(
    ...
    try:
        ...
        print(f"[GDrive] アップロード完了 filename={filename!r} file_id={file_id!r}")
        return file_id
    except Exception as e:
        print(f"[GDrive] アップロードエラー filename={filename!r}: {e}")
        # フォールバック
        return _save_local(content, filename)
...
def upload_json(
    ...
    try:
        ...
        file_id: str = result["id"]
        print(f"[GDrive] JSON アップロード完了 filename={filename!r} file_id={file_id!r}")
        return file_id
    except Exception as e:
        print(f"[GDrive] JSON アップロードエラー filename={filename!r}: {e}")
        return _save_local(json_content, filename)
...
def create_subfolder(
    ...
    service = _build_drive_service()
    if service is None:
        print(f"[GDrive] サービス未接続。フォルダ作成スキップ: {name!r}")
        return None
    ...
    try:
        ...
        print(f"[GDrive] フォルダ作成完了 name={name!r} folder_id={folder_id!r}")
        return folder_id
    except Exception as e:
        print(f"[GDrive] フォルダ作成エラー name={name!r}: {e}")
        return None
